...
class Config:
    ...
    hand_roi_padding_ratio: float = 0.15
...
def get_active_person_roi(
    frame: np.ndarray,
    person: Person,
    config: Config,
) -> tuple[np.ndarray, tuple[int, int]]:
    """Recorta a pessoa ativa com margem para mãos e braços."""
    frame_height, frame_width = frame.shape[:2]
    x1, y1, x2, y2 = person.bbox

    padding_x = int((x2 - x1) * config.hand_roi_padding_ratio)
    padding_y = int((y2 - y1) * config.hand_roi_padding_ratio)
...
def roi_point_to_frame(
    ...
    """Converte coordenadas normalizadas do ROI para o frame original."""
    return (
        int(normalized_x * roi_width) + offset_x,
        int(normalized_y * roi_height) + offset_y,
    )


def process_active_person_hands(
    frame: np.ndarray,
    hand_result,
    ...
    """Processa exclusivamente as mãos encontradas no ROI da pessoa ativa."""
    ...
    for index, landmarks in enumerate(hand_result.hand_landmarks):
        ...
        wrist = (
            int(np.clip(wrist[0], 0, frame_width - 1)),
            int(np.clip(wrist[1], 0, frame_height - 1)),
        )

        normalized_x, normalized_y = normalize_point(
            wrist[0], wrist[1], frame_width, frame_height,
        )
        ...
        draw_hand_skeleton(frame, points, point_color, line_color)
        draw_hand_status(frame, wrist, openness, config)
...
def detect_active_person_hands(
    frame: np.ndarray,
    ...
    """Executa MediaPipe somente no ROI da pessoa ativa."""
    if active_person is None:
        return

    roi, offset = get_active_person_roi(
        frame,
        active_person,
        config,
    )
...
def process_frame(
    ...
    """Executa YOLO, seleciona a pessoa ativa e processa seu ROI."""
    ...
    for person in people:
        person.left_hand = None
        person.right_hand = None

    detect_active_person_hands(
        frame,
        landmarker,
        active_person,
        timestamp_ms,
...
def main() -> None:
    ...
    print("MediaPipe: processando somente o ROI da pessoa ativa.")
    print(f"Margem do ROI das mãos: {config.hand_roi_padding_ratio:.0%}")
